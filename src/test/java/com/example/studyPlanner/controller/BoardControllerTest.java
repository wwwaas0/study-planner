...
public class BoardControllerTest {
    ...
    void getBoards() throws Exception {
        //given
        GetBoardListRes board1 = new GetBoardListRes();
//        board1.setId(1L);
        board1.setName("고등학생");

        GetBoardListRes board2 = new GetBoardListRes();
//        board2.setId(2L);
        board2.setName("취준생");

        List<GetBoardListRes> boards = new ArrayList<>();
        boards.add(board1);
        boards.add(board2);

        given(boardService.getBoardNames()).willReturn(boards);
        ...
        actions.andExpect(status().isOk())
                .andExpect(view().name("board/name-list"));
    }
...
//        given(boardService.deleteBoard(board.getId()));; //mocking
        ...
        Long boardId = 1L;
        ResultActions actions = mockMvc.perform(
                get("/boards/delete/{boardId}", boardId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON));
